...
export const FarmManager = ({ session, initialFields = [], initialWarehouse = [] }) => {
  ...
  const handleSaveField = async (e) => {
    ...
    const res = await fetch('/api/fields', { method: 'POST', body: JSON.stringify(data) });
    if (res.ok) {
  ...
  const WeatherWidget = () => (
    <div className="bg-slate-900 text-white p-2 px-6 flex justify-between items-center text-[10px] font-bold tracking-widest uppercase">
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => signOut({ callbackUrl: '/login' })} className="hover:text-red-400 transition-colors flex items-center gap-1 border-l border-slate-700 pl-4">
    ...
    <div className="bg-slate-100 min-h-screen pb-20 font-sans text-slate-900 print:bg-white print:pb-0">
      ...
      <div className="bg-white border-b border-gray-300 sticky top-0 z-20 shadow-md print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4 text-sm font-medium uppercase tracking-widest text-slate-400 text-[10px]">
        ...
        {activeTab === 'fields' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setModalOpen('field')} className="btn-primary">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {fields.map(f => (
                <div key={f.id} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-all group">
                   <div className="flex justify-between items-start">
                      <div className="text-[10px] font-bold text-slate-400 uppercase">{f.parcelNumber}</div>
                      <div className="text-right font-black text-2xl text-slate-800">{f.area} ha</div>
                   </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
               <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-200 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                    <tr><th className="p-4">Środek</th><th className="p-4">Ilość</th><th className="p-4">Partia</th><th className="p-4 text-right">Akcje</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-bold text-sm">
                    {warehouse.map(w => (
                      <tr key={w.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4 text-right"><Trash2 className="w-4 h-4 text-red-400 inline cursor-pointer"/></td>
                      </tr>
                    ))}
                  </tbody>
               </table>
      ...
      <style jsx global>{`
        ...
        .btn-primary { background: #16a34a; color: white; font-weight: 900; padding: 0.75rem 1.5rem; border-radius: 0.75rem; display: flex; align-items: center; gap: 0.5rem; text-transform: uppercase; font-size: 0.7rem; transition: all 0.2s; }
